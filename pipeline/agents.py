...
원칙:
- 이건 실제 돈이 걸린 판단이다. 근거 없는 낙관은 금물이고, 과거 낙관적 판단이 손실을 낸 전례도 있다.
  그러나 습관적으로 WATCH/PASS로 도피하지도 마라 — 판단을 회피하는 것도 비용이다. 팩트시트의
  기술적 구조와 근거가 매수측 논리를 분명히 뒷받침하고 리스크가 관리 가능한 수준이면 BUY를 준다.
- 확신의 정도는 confidence(0-100)로 정직하게 표현한다. 근거가 탄탄할수록 높게, 불확실할수록 낮게.
...
- verdict는 STRONG_BUY / BUY / WATCH / PASS 중 하나로 명확히 판정한다.
  · 상승 구조가 뚜렷하고 매수 논리가 리스크를 능가 → BUY (근거가 특히 강하면 STRONG_BUY)
  · 방향은 나쁘지 않으나 진입 타이밍·근거가 약함 → WATCH
  · 상승 구조가 무너졌거나 리스크가 매수 논리를 압도 → PASS

반드시 아래 JSON만 출력한다.
{"verdict":"STRONG_BUY|BUY|WATCH|PASS",
 "confidence":0-100,
 "sector":"반도체|2차전지|바이오|자동차|금융|조선·방산|IT·플랫폼|소비재|건설·기계|에너지|거시·환율|증시일반 중 이 종목의 주력 사업에 가장 가까운 하나",
