...
MIN_CONVICTION = 50        # Judge confidence 이 값 이상만 신규 편입 (2026-07-16 완화: 60→50, 추천 빈도 ↑)
...
MAX_CANDIDATES_TO_AI_KR = 12               # 한국 AI 분석 후보 수 (2026-07-16: 8→12, 추천 빈도 ↑)
MAX_CANDIDATES_TO_AI_US = 12               # 미국 AI 분석 후보 수 (2026-07-16: 8→12, 추천 빈도 ↑)

# ── 관찰가능성 / 실패 처리 ──────────────────────────────────
# AI 분석 실패율이 이 값 이상이면 "후보 0개"로 조용히 발행하지 않고, 명확히 "분석 실패"
# 상태로 스냅샷·사이트에 표시하고 런을 실패(빨간불)로 끝낸다 (2026-07-16 크레딧 소진으로
# 이틀간 아무 신호 없이 사이트가 안 올라간 사고 후 추가).
AI_FAILURE_ABORT_RATE = 0.8
